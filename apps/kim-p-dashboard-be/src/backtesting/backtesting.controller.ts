...
export class BacktestingController {
  ...
  async uploadCsvData(
    ...
    try {
      ...
      await this.historicalPriceService.deleteHistoricalPrices(body.symbol);
      await this.historicalPriceService.saveHistoricalPrices(parsedData);

      return {
        success: true,
        message: `${body.symbol}: ${parsedData.length}개의 가격 데이터 업로드 완료`,
        data: {
          symbol: body.symbol,
